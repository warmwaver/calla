    ...
    def _html(self, digits=2):
        ...
        if self.type == '2' or self.type == '4':
            ...
            yield self.format_conclusion(ok,
                                         ...
                                         self.format('eqr2', digits, eq='{}√(345/fy)'.format(12 if self.type ==
                                                     '2' else 30), omit_name=True),
        ...
        yield self.format_conclusion(ok,
                                     ...
                                     self.format('eqr', digits=digits,
                                                 eq='{}√(345/fy)'.format(factor[self.type]), omit_name=True),
                                     '{}满足规范要求。'.format('' if ok else '不'))
    ...
    def _html(self, digits=2):
        ...
        if case == '2':
            ...
            yield '{} {} {}，{}满足刚性加劲肋要求。'.format(
                ...
                '' if rigid else '不')
            if not rigid:
                eq = '((1+α<sup>2</sup>)<sup>2</sup>+n*γl)/α<sup>2</sup>/(1+n*δl)' if self.α <= self.α0 else\
                    '2*(1+sqrt(1+n*γl))/(1+n*δl)'  # (B.0.1-3~4)
        elif case == '4':
            yield self.format('D', digits, eq='E·t<sup>3</sup>/12/(1-υ<sup>2</sup>)')
            yield self.format('Il', digits)
            yield self.format('It', digits)
            yield self.format('α0', digits, eq='(1+n*γl)<sup>(1/4)</sup>')
            yield '首先验证条件(B.0.1-7)，取{}'.format(self.format('α', digits, value=self._α, eq='at/b'))
            yield self.format('δl', digits, eq='Al/b/t')
            eq = '1/n·(4·n<sup>2</sup>·(1+n·δl)·α<sup>2</sup>-(α<sup>2</sup>+1)<sup>2</sup>)' \
                if self.α <= self.α0 else '1/n·((2·n<sup>2</sup>·(1+n·δl)-1)<sup>2</sup>-1)'
            yield self.format('γl_', digits, value=self._γl_, eq=eq)
            rigid = self.γl >= self.γl_
            ...
            if condition:
                ...
                yield self.format('γl_', digits, eq=eq)
                if rigid is False:
                    eq = '((1+α<sup>2</sup>)<sup>2</sup>+n*γl+α<sup>4</sup>*(nt+1)*γt)/α<sup>2</sup>/(1+n*δl)' \
                        if self.α <= self.α0 else '2*(1+sqrt((1+n*γl)*(1+(nt+1)*γt)))/(1+n*δl)'  # (B.0.1-8)
            yield '{} {} {}，{}满足刚性加劲肋要求。'.format(
                self.format('γl', digits, eq='E·Il/b/D'), '≥' if rigid else '&lt;',
                self.format('γl_', digits=digits, omit_name=True),
                '' if rigid else '不')
...
class stability_bending(abacus):
    ...
    def fsolve(γ0, fy, fd, My, Mz, Wyeff, Wzeff, Mcry, Mcrz, βmy=1, βmz=1, α=0.35):
        def fε0(α, λ): return α*(λ-0.2)
        def fχ(λ, ε0): return 0.5*(1+(1+ε0)/λ**2-sqrt((1+(1+ε0)/λ**2)**2-4/λ**2))
...
class stability_bending_compression(abacus):
    ...
    def solve(self):
        def fχ(λ, ε0): return 0.5*(1+(1+ε0)/λ**2-sqrt((1+(1+ε0)/λ**2)**2-4/λ**2))  # (A.0.1-1)
        def fε0(α, λ): return α*(λ-0.2)  # (A.0.1-3)

        self.validate('positive', 'fy', 'fd', 'Ncry', 'Ncrz', 'Mcry', 'Mcrz',
                      'Aeff', 'Wyeff', 'Wzeff', 'χy', 'χz', 'χLTy', 'χLTz')
...
class web_rib(abacus):
    ...
    def rib_space(a, hw, tw, σ, τ, fvd, C1, C2, C3):
        ...
        def feql(a, hw, tw, σ, τ, fvd, C1, C2, C3): return (hw/100/tw)**4*((σ/C1)**2+(τ/(C2+C3*(hw/a)**2))**2)
...
class support_rib(abacus):
    ...
    def solve(self):
        def feql1(γ0, Rv, As, Beb, tw): return γ0*Rv/(As+Beb*tw)  # (5.3.4-1)
        def feql2(γ0, Rv, As, Bev, tw): return γ0*2*Rv/(As+Bev*tw)  # (5.3.4-1)
...
class stability_reduction_factor(abacus):
    ...
    def solve(self):
        def fε0(α, λ): return α*(λ-0.2)
        def fχ(λ, ε0): return 0.5*(1+(1+ε0)/λ**2-sqrt((1+(1+ε0)/λ**2)**2-4/λ**2))
        def fλ_(λ, fy, E): return λ/pi*sqrt(fy/E)
