...
from calla.GB.compressive_capacity import \
    eccentric_compression as gb_eccentric_compression
from calla.GB.flexural_capacity import fc_rect as gb_fc_rect
...
class fc_T(fc_rect, material_base):
    ...
    def solve(self):
        ...
        a_s=self.a_s; as_=self.as_; a_=self.a_
        fcd=self.fcd; fsd=self.fsd; As=self.As; fsd_=self.fsd_; As_=self.As_
        fpd=self.fpd; fpd_=self.fpd_; Ap=self.Ap; Ap_=self.Ap_; ap=self.ap
    ...
    def _html(self,digits=2):
        ...
        if self._same_as_rect:
            yield '按宽度为{}的矩形截面计算，按式(5.2.2-2)计算受压区高度。'.format(self.para_attrs('bf_').symbol)
...
class eccentric_compression_Ishape(eccentric_compression):
    ...
    __inputs__.update(eccentric_compression.__inputs__)
...
class eccentric_tension(abacus, material_base):
    ...
    def _html(self, digits = 2):
        ...
        yield '按{}偏心受拉构件计算'.format('大' if self.large_eccentric else '小')
        if not self.large_eccentric:
            ...
            yield '对式(5.4.2-3)两边取等计算得：'
            ok = self.x<self.xb
            yield '{} {} {}'.format(self.format('x', digits), '&lt;' if ok else '&gt;', self.format('xb', omit_name = True))
            ...
            if ok:                
                ...
                yield '{} {} {}，{}满足规范式(5.4.2-4)要求。'.format(
                    self.format('γ0Nde',digits=digits,eq='γ0*Nd*e'), 
                    '&le;' if ok else '&gt;', 
                    self.format('Mu',digits=digits,omit_name=True, eq='fcd*b*x*(h0-x/2)+fsd_*As_*(h0-as_)+(fpd_-σp0_)*Ap_*(h0-ap_)'),
                    '' if ok else '不')
...
class torsion(abacus, material_base):
    ...
    def solve(self):
        ...
        self.βt = self.fβt(Vd,Td,b,self.h0,Wt) if Td>0 else 0
        if self.βt < 0.5:
            self.βt = 0.5
        if self.βt >1.0:
            self.βt = 1.0
...
class local_pressure(abacus, material_base):
    ...
    __inputs__ = [
        ...
        material_base.concrete_input,
        ('fcd','<i>f</i><sub>cd</sub>','N/mm<sup>2</sup>',22.4,'混凝土轴心抗压强度设计值'),
        ('fcuk','<i>f</i><sub>cu,k</sub>','MPa',50,'混凝土立方体抗压强度标准值','取混凝土标号'),
        material_base.rebar_input,
        ('fsd','<i>f</i><sub>sd</sub>','MPa',330,'钢筋抗拉强度设计值'),
