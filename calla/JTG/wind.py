...
class wind_girder(abacus):
    ...
    table_GV = (
        # (L, A, B, C, D)
        (20, 1.29, 1.35, 1.49, 1.56),
        (60, 1.28, 1.33, 1.48, 1.54),
        (100, 1.26, 1.31, 1.45, 1.51),
        (200, 1.24, 1.29, 1.41, 1.47),
        (300, 1.23, 1.27, 1.39, 1.44),
        (400, 1.22, 1.26, 1.37, 1.42),
        (500, 1.21, 1.25, 1.36, 1.41),
        (650, 1.2, 1.24, 1.34, 1.39),
        (800, 1.19, 1.23, 1.33, 1.37),
        (1000, 1.18, 1.22, 1.31, 1.35),
        (1200, 1.17, 1.21, 1.3, 1.34),
        (1500, 1.16, 1.2, 1.29, 1.32),
        (2000, 1.15, 1.18, 1.26, 1.3)
    )

    table_5_3_2_1 = (
        # (矩形与H形截面构件, 圆柱形构件dUd<=6, 圆柱形构件dUd>6)
        (1.9, 1.2, 0.7),
        (1.8, 1.2, 0.8),
        (1.7, 1.2, 0.8),
        (1.7, 1.1, 0.8),
    ...
    def solve(self):
        ...
        # 将地表类别的值A、B、C、D分别对应索引1、2、3、4
        col = {'A': 1, 'B': 2, 'C': 3, 'D': 4}.get(self.地表类别.upper(), 0)
        if col == 0:
            raise ValueError("错误的地表类别值")

        self.GV = numeric.query_table(self.table_GV, self.L, col)
